...
// Bodysuit color for the photo-based body map. Default ("blush") is picked to match the
// actual pale pink fabric in the reference photos, so it reads as a no-op recolor for anyone
// who never opens this picker — only a deliberate choice visibly shifts the fabric color.
const BODYSUIT_COLORS = [
  { key: "blush", label: "Blush (default)", hex: "#F0C9C7" },
  { key: "lavender", label: "Lavender", hex: "#C9BFE0" },
  { key: "sage", label: "Sage", hex: "#AEBFA0" },
  { key: "sky", label: "Sky blue", hex: "#AECBDD" },
  { key: "cream", label: "Cream", hex: "#EADFC4" },
  { key: "plum", label: "Plum", hex: "#9C6E86" },
  { key: "black", label: "Black", hex: "#3A3632" },
];
...
// Body map now renders on top of real reference photos (front + back) instead of a drawn
// silhouette, so realism doesn't get traded away for the sake of an illustration. Tap zones
// are plotted as ellipses in percent-of-image coordinates, derived by measuring the actual
// silhouette in each photo (row-by-row width scan against the plain background) rather than
// reusing the old vector body's coordinate space, so they land on the real figure.
const FRONT_PHOTO_HITZONES = [
  { k: "head", cx: 50, cy: 12, rx: 13, ry: 8 },
  { k: "neck", cx: 50, cy: 21, rx: 6, ry: 2.6 },
  { k: "shoulderL", cx: 31, cy: 26.5, rx: 10, ry: 4 },
  { k: "shoulderR", cx: 69, cy: 26.5, rx: 10, ry: 4 },
  { k: "chest", cx: 50, cy: 37, rx: 15, ry: 6 },
  { k: "abdomenRUQ", cx: 41, cy: 44, rx: 9, ry: 5 },
  { k: "abdomenLUQ", cx: 59, cy: 44, rx: 9, ry: 5 },
  { k: "abdomenRLQ", cx: 39, cy: 52.5, rx: 10, ry: 5 },
  { k: "abdomenLLQ", cx: 61, cy: 52.5, rx: 10, ry: 5 },
  { k: "armL", cx: 21, cy: 40, rx: 6.5, ry: 12 },
  { k: "armR", cx: 79, cy: 40, rx: 6.5, ry: 12 },
  { k: "handL", cx: 15, cy: 53.5, rx: 5.5, ry: 4.2 },
  { k: "handR", cx: 85, cy: 53.5, rx: 5.5, ry: 4.2 },
  { k: "legL", cx: 38, cy: 68, rx: 8.5, ry: 10 },
  { k: "legR", cx: 62, cy: 68, rx: 8.5, ry: 10 },
  { k: "kneeL", cx: 39, cy: 80, rx: 6, ry: 3.5 },
  { k: "kneeR", cx: 61, cy: 80, rx: 6, ry: 3.5 },
  { k: "shinL", cx: 39, cy: 88, rx: 5, ry: 6 },
  { k: "shinR", cx: 61, cy: 88, rx: 5, ry: 6 },
  { k: "footL", cx: 38, cy: 95.5, rx: 5.5, ry: 3 },
  { k: "footR", cx: 62, cy: 95.5, rx: 5.5, ry: 3 },
];
...
const BACK_PHOTO_HITZONES = [
  ...
  { k: "shoulderL", cx: 31, cy: 26.5, rx: 10, ry: 4 },
  { k: "shoulderR", cx: 69, cy: 26.5, rx: 10, ry: 4 },
  { k: "upperBackL", cx: 38, cy: 32, rx: 11, ry: 5 },
  { k: "upperBackR", cx: 62, cy: 32, rx: 11, ry: 5 },
  { k: "lowerBack", cx: 50, cy: 43, rx: 15, ry: 6 },
  { k: "glutesL", cx: 39, cy: 55, rx: 10, ry: 6 },
  { k: "glutesR", cx: 61, cy: 55, rx: 10, ry: 6 },
  { k: "armL", cx: 21, cy: 40, rx: 6.5, ry: 12 },
  { k: "armR", cx: 79, cy: 40, rx: 6.5, ry: 12 },
  { k: "handL", cx: 15, cy: 53.5, rx: 5.5, ry: 4.2 },
  { k: "handR", cx: 85, cy: 53.5, rx: 5.5, ry: 4.2 },
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {BODYSUIT_COLORS.map((b) => (
          <button key={b.key} onClick={() => setBodysuitColor(b.key)} aria-label={b.label}
            ...
            {bodysuitColor === b.key && <Check size={14} color="#fff" style={{ filter: "drop-shadow(0 0 1px rgba(0,0,0,0.5))" }} />}
          </button>
        ))}
      </div>
  ...
  return (
    <div className={motionOff ? "whimsy-root reduce-motion" : "whimsy-root"} style={{ background: "radial-gradient(900px 420px at -10% -5%, #F8DCE4 0%, rgba(248,220,228,0) 60%), radial-gradient(700px 360px at 110% -2%, #F4E4CE 0%, rgba(244,228,206,0) 55%), #FBF1EC", minHeight: "100vh", color: COLORS.ink, fontFamily: BODY }}>
      <style>{`
        ...
        .bodypart-photo{cursor:pointer;transition:fill .18s ease,stroke .18s ease;outline:none;}
        .bodypart-photo:hover{fill:rgba(122,46,77,0.16);}
        .bodypart-photo:active{opacity:.8;}
          ...
          <section className="tab-in">
            ...
            <div className="mb-6">
              ...
              <BodyMap selected={painAreas} setSelected={setPainAreas} skin={activeSkin} shape={activeBodyShape} hairStyle={hairStyle} hairColorHex={activeHairColor.hex}
                bodysuitColorHex={activeBodysuitColor.hex} bodysuitIsDefault={bodysuitColor === "blush"} skinIsDefault={skinTone === "blush"}
...
function BodyMap({ selected, setSelected, skin, shape, hairStyle, hairColorHex, bodysuitColorHex, bodysuitIsDefault, skinIsDefault, onOpenAppearance }) {
  ...
  const suffix = BODY_PHOTO_VARIANT[shape?.key] ?? "";
  const photoSrc = `/assets/body-map/whimsy-body-${view}${suffix}.png`;
  ...
  return (
    <div>
      ...
      <div className="rounded-3xl relative overflow-hidden"
        ...
        <span className="whimsy-sway absolute" style={{ left: 12, bottom: 6, opacity: 0.3, zIndex: 3 }}><Sprig size={22} /></span>
        <span className="whimsy-float absolute" style={{ right: 14, top: 8, opacity: 0.5, zIndex: 3 }}><Sparkles size={13} style={{ color: COLORS.gold }} /></span>
        <div className="relative mx-auto" style={{ width: "78%", aspectRatio: "927 / 1696" }}
          aria-label={`Body map, ${view} view. Tap where it aches.`}>
          ...
          {hairStyle && hairStyle !== "bald" && view === "front" && (
            <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
              {(() => {
                ...
                // a "circle" draws as a tall cone -- which is exactly what the first pass
                // looked like. ASPECT converts an x-radius into the y-radius that renders
                // as the same number of on-screen pixels.
          ...
          <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
            {hitzones.map((z) => {
              const on = isOn(z.k);
              const label = BODY_MAP.find((b) => b.k === z.k)?.label || z.k;
              return (
                <ellipse key={z.k} cx={z.cx} cy={z.cy} rx={z.rx} ry={z.ry}
                  onClick={() => toggle(z.k)} aria-label={label}
                  role="button" tabIndex={0} aria-pressed={on}
                  onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); toggle(z.k); } }}
                  className="bodypart-photo"
                  fill={on ? "rgba(122,46,77,0.38)" : "transparent"}
                  stroke={on ? COLORS.plumDark : "transparent"}
                  strokeWidth={on ? 0.6 : 0} vectorEffect="non-scaling-stroke" />
              );
            })}
          </svg>
        </div>
